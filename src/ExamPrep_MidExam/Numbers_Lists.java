...
//                .replace(",", ""));
    ...
    private static List<Integer> findTopFive(List<Integer> biggerThanAverage) {
        ...
        while (true) {
            ...
            //remove all occurences of the added number from the biggest list
            for (int i = 0; i < biggerThanAverage.size(); i++) {
                if (biggerThanAverage.get(i) == biggest) {
                    biggerThanAverage.remove(i);
                    i--;// Decrement i to recheck the current index after removal, because the elements shift left after each removal
//                    System.out.println(biggerThanAverage);
                }
            }
        }
